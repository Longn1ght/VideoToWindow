...
int WINAPI WinMain(HINSTANCE hinstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow)
{
	...
	mainwndclassex.lpfnWndProc = VTWControlProc;
	mainwndclassex.cbClsExtra = 0;
	mainwndclassex.cbWndExtra = 0;
	mainwndclassex.hInstance = g_hInstance;
	mainwndclassex.hIcon = LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_VTW));
	mainwndclassex.hCursor = LoadCursor(NULL, IDC_ARROW);
	mainwndclassex.hbrBackground = static_cast<HBRUSH>(GetStockObject(WHITE_BRUSH));
	mainwndclassex.lpszMenuName = NULL;
	mainwndclassex.lpszClassName = TEXT("VTW_MAIN");
	mainwndclassex.hIconSm = LoadIcon(g_hInstance, MAKEINTRESOURCE(IDI_VTW));
	MSG msg;
	RECT ClientRect = { 0, 0, 1280, 550 };
	AdjustWindowRectEx(&ClientRect, WS_OVERLAPPEDWINDOW, FALSE, 0);
	int winWidth = ClientRect.right - ClientRect.left;
	int winHeight = ClientRect.bottom - ClientRect.top;
	RegisterClassEx(&mainwndclassex);
	HWND hwnd = CreateWindowEx(WS_EX_ACCEPTFILES, TEXT("VTW_MAIN"), TEXT("Video To Window"),
		WS_OVERLAPPEDWINDOW,
		0, 0, 
		winWidth, winHeight, 
